import React, { useEffect, useState } from 'react'
import axios from 'axios'
import AdminNavbar from '../../components/navs/AdminNavbar.jsx'

const PendingVaccinesPage = () => {
  const [requests, setRequests] = useState([])

  const fetchRequests = async () => {
    const res = await axios.get('http://localhost:4000/api/admin/vaccines/requests', { withCredentials: true })
    setRequests(res.data)
  }

  const approve = async (id) => {
    await axios.put(`http://localhost:4000/api/admin/vaccines/approve/${id}`, {}, { withCredentials: true })
    fetchRequests()
  }

  useEffect(() => { fetchRequests() }, [])

  return (
    <>
      <AdminNavbar />
      <div className='min-h-screen bg-black text-white p-6'>
        <h2 className='text-2xl mb-4'>Pending Vaccine Requests</h2>
        <div className='grid gap-4'>
          {requests.map(r => (
            <div key={r._id} className='bg-white text-black p-4 rounded'>
              <div className='font-semibold'>{r.name}</div>
              <div className='text-sm'>{r.description}</div>
              <button onClick={()=>approve(r._id)} className='mt-2 bg-black text-white px-3 py-1 rounded'>Approve</button>
            </div>
          ))}
          {requests.length === 0 && <div>No pending requests</div>}
        </div>
      </div>
    </>
  )
}

export default PendingVaccinesPage